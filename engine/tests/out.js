...
// engine/tests/test_golden_parity.ts
...
function calculateGoldenTest(inputs, policy2) {
  ...
  if (scenario) {
    if (scenario === "DEMAND_DROP_20" && unitsSold != null) {
      unitsSold = Math.floor(unitsSold * 0.8);
      result.simulated_monthly_units_sold = unitsSold;
    } else if (scenario === "RAW_MATERIAL_UP_20" && varCost != null) {
      varCost = varCost.mul(1.2);
      result.simulated_variable_cost_per_unit = Number(roundCurrency(varCost).toFixed(2));
      result.simulated_monthly_units_sold = unitsSold;
    } else if (scenario === "TRANSPORT_COST_SPIKE_50") {
      const transport = new Decimal(inputs.monthly_transport_cost || 0).mul(1.5);
      fixedCost = new Decimal(inputs.monthly_labour_cost || 0).add(inputs.monthly_rent || 0).add(transport).add(inputs.monthly_other_fixed_cost || 0);
      result.simulated_monthly_transport_cost = Number(roundCurrency(transport).toFixed(2));
      result.simulated_monthly_units_sold = unitsSold;
    }
  }
  ...
  if (unitsSold != null && price != null && varCost != null && fixedCost != null) {
    ...
    const prefix = scenario ? "simulated_" : "";
  ...
  if (affordableOverride != null && reqLoan != null && maxScheme != null) {
    ...
    if (inputs.maximum_affordable_emi == null && affordableOverride == null && maxScheme.equals(125e3)) {
    }
  }
  ...
  if (bizDscr != null && hhPostRatio != null) {
    const minDscr = new Decimal(inputs.minimum_required_dscr || policy2.minimum_required_dscr);
    const maxHhDebt = new Decimal(inputs.maximum_household_debt_ratio || policy2.maximum_household_debt_ratio);
...
// engine/tests/test_golden_parity.ts
var specDir = path.join(__dirname, "../../finance-spec");
var testsFile = path.join(specDir, "golden-tests.json");
var policyFile = path.join(specDir, "calculation-policy.json");
var goldenTests = JSON.parse(fs.readFileSync(testsFile, "utf8"));
var policy = JSON.parse(fs.readFileSync(policyFile, "utf8"));
var passed = 0;
var failed = 0;
for (const testCase of goldenTests) {
  const result = calculateGoldenTest(testCase.inputs, policy);
  let testPassed = true;
  for (const [key, expectedValue] of Object.entries(testCase.expected)) {
    const resVal = result[key];
    if (expectedValue === null) {
      if (resVal !== null && resVal !== void 0) {
